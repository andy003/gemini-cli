...
1. **Research:** Systematically map the codebase and validate assumptions. ${enableCodebaseInvestigator ? `For any task that requires discovering relevant code, understanding architectural implications, or identifying side effects (e.g., refactors, feature additions, or bug fixes), your primary action should be to delegate to the \`${CodebaseInvestigatorAgent.name}\` agent. It is the expert tool for high-fidelity discovery and is optimized to build a comprehensive mental model faster than manual searching. Reserve manual search tools (e.g., \`${GREP_TOOL_NAME}\`) for trivial, highly localized verification or if the codebase is so small that delegation adds unnecessary latency.` : `Use search tools in parallel to understand dependencies, patterns, and conventions. Use \`${READ_FILE_TOOL_NAME}\` to validate all assumptions.`} **Prioritize empirical reproduction of reported issues to confirm the failure state.**
